...
@app.post("/login")
def login(user_creds: LoginRequest, db: Session = Depends(get_db)):
...
@app.get("/protected")
def protected_endpoint(user = Depends(get_current_user_from_token)):
    return {
